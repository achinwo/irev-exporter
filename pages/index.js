            ...
            !_.isEmpty(wardStat) ?
                <>
                    <Chip label={`${wardStat.wardCount}${ward.stats ? '/' + ward.stats.resultCount : ''}`}
                          ...
                          variant={_.toInteger(wardStat.wardCount) >= ward.stats?.resultCount ? 'filled' : "outlined"}
                          size="small"/>
                    <Chip sx={{maxWidth: 100}} title={`Last contributor ${wardStat.lastContributorUsername}`}
                          icon={<FaceIcon/>} label={wardStat.lastContributorUsername} color="primary" variant="outlined"
                          size="small"/>
                </>
...
const App = () => {
    const [selectedElectionSession, setSelectedElectionSession] = useState(null);
    ...
    useEffect(async () => {
        const res = await axios.get(`/api/states/${selectedElectionSession}/stats`);
        let rows = [];
    ...
    useEffect(async () => {
        ...
        try {
            ...
            const response = await axios.get(`/api/pus/${selectedElectionSession}/${selectedWard._id}`);
        ...
        <>
            ...
            <Box sx={{display: "flex"}}>
                    ...
                    <Toolbar>
                        ...
                        <Box display="flex" alignItems="center" style={{width: "100%"}}>
                            ...
                            <Stack direction={"row"}>
                                ...
                                <Select
                                    onChange={(event) =>
                                        setSelectedElectionSession(
                                            event.target.value === ""
                                                ? null
                                                : _.toInteger(event.target.value)
                                        )
                                    }
                                ...
                                <FormControl sx={{m: 1, minWidth: 120}} size="small">
                                    <InputLabel>Election Session</InputLabel>
                ...
                <Box component="nav">
                    ...
                    >
                        <DrawerView handleDrawerToggle={handleDrawerToggle} state={selectedState} lga={selectedLga}
                                    ward={selectedWard} pu={selectedPu} setWard={setWard} setLga={setSelectedLga}
                                    stats={stats}/>
                    </Drawer>
                    <Drawer
                        variant="permanent"
                    ...
                    >
                        <DrawerView handleDrawerToggle={handleDrawerToggle} state={selectedState} lga={selectedLga}
                                    pu={selectedPu} setWard={setWard} setLga={setSelectedLga} stats={stats}/>
                    </Drawer>
                </Box>
